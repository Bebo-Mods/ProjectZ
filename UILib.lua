...
-- ── CRITICAL: ONLY use gethui() - NO FALLBACKS ─────────────────
-- If gethui() doesn't exist or fails, the script WILL NOT load
-- This prevents any detectable fallback to CoreGui/PlayerGui

local hiddenContainer = gethui()
if not hiddenContainer then
    error("MacLib: gethui() failed - no hidden container available. Script will not load to avoid detection.")
end
...
function MacLib:Window(Settings)
    ...
    base.Name = HttpService:GenerateGUID(false)
    ...
    sidebar.Name = HttpService:GenerateGUID(false)
    ...
    -- Acrylic blur setup (hidden in gethui)
